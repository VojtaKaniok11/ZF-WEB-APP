...
export default function EmployeeProfilePage() {
    ...
    const pnRaw = searchParams.get("pn");
    ...
    useEffect(() => {
        if (!pnRaw || pnRaw === 'undefined') {
            setError("Neplatné osobní číslo.");
            setLoading(false);
            return;
        }
        ...
        const fetchData = async () => {
            try {
                const [empRes, trnRes, medRes, ooppRes, iluoRes] = await Promise.all([
                    fetch(`${apiUrl}/employees/${personalNumber}`),
                    fetch(`${apiUrl}/trainings/${personalNumber}`),
                    fetch(`${apiUrl}/medical/${personalNumber}`),
                    fetch(`${apiUrl}/oopp/${personalNumber}`),
                    fetch(`${apiUrl}/iluo/${personalNumber}`),
                ]);

                const empJson = await empRes.json();
                if (!empJson.success) {
                    setError("Zaměstnanec nenalezen.");
                    setLoading(false);
                    return;
                }
                ...
                setTrainings(trnJson.success ? trnJson.data : []);

                const medJson = await medRes.json();
                setMedicals(medJson.success ? medJson.data : []);

                const ooppJson = await ooppRes.json();
                setOopp(ooppJson.success ? ooppJson.data : []);

                const iluoJson = await iluoRes.json();
                setIluo(iluoJson.success ? iluoJson.data : []);
            } catch (err) {
    ...
    if (error || !employee) {
        return (
            <div className="mx-auto max-w-5xl px-4 py-12 text-center">
                <p className="text-gray-500">{error || "Při načítání profilu nastala chyba."}</p>
        ...
        <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                <DashboardCard
                    ...
                    linkHref={`/trainings/detail?pn=${employee.personalNumber}`}
                ...
                <DashboardCard
                    ...
                    linkHref={`/medical/detail?pn=${employee.personalNumber}`}
                    linkLabel="Zobrazit vše"
                ...
                <DashboardCard
                    title="OOPP"
                    count={oopp.length}
                    linkHref={`/oopp/detail?pn=${employee.personalNumber}`}
                    linkLabel="Zobrazit vše"
                ...
                <DashboardCard
                    title="ILUO"
                    count={iluo.length}
                    linkHref={`/iluo/detail?pn=${employee.personalNumber}`}
                    linkLabel="Zobrazit vše"
